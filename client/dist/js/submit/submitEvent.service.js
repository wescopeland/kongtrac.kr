(function() {
    ...
    function submitEventService(
        ...
        function handleEventGames(inputEventGames, inputEventReference) {
            ...
            var dbGames = $firebaseArray(_fbRef.child('games'));
                ...
                dbGames.$loaded().then(function() {
                    inputEventGames.forEach(function(game) {
                        ...
                        dbGames.forEach(function(dbGame) {
                            if (
                                dbGame.player === game.player &&
                                dbGame.score === Number(game.score)
                            ) {
                                alreadyExists = true;
                        ...
                        if (!alreadyExists) {
                            ...
                            dbGames
                                .$add(newGameProperties)
                                .then(function(newGameReference) {
                                    ...
                                    playerGamesArray.$loaded().then(function() {
                                        playerGamesArray.$add(
                                            newGameReference.key
                                        );
                                    });
                                });
                        }
        ...
        function submitEvent(inputEventProperties, inputEventGames) {
            var eventList = $firebaseArray(_fbRef.child('events'));
            ...
            eventList.$loaded().then(function() {
                ...
                inputEventGames.forEach(function(game) {
                    if (game.winnings && game.winnings > 0) {
                        newEvent.winnings[camelize(game.player)] =
                            game.winnings;
                    }
                });
