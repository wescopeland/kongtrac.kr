(function() {
    ...
    function SubmitGameController(
        ...
        function handleCommit() {
            ...
            if (vm.inputIsDayUnknown && !vm.inputIsMonthUnknown) {
                ...
                if (gamePropertiesObject.date.indexOf('Jan') > -1) {
                    gamePropertiesObject.date =
                        '01/15/' + gamePropertiesObject.date.split(' ')[1];
                }

                if (gamePropertiesObject.date.indexOf('Feb') > -1) {
                    gamePropertiesObject.date =
                        '02/15/' + gamePropertiesObject.date.split(' ')[1];
                }

                if (gamePropertiesObject.date.indexOf('Mar') > -1) {
                    gamePropertiesObject.date =
                        '03/15/' + gamePropertiesObject.date.split(' ')[1];
                }

                if (gamePropertiesObject.date.indexOf('Apr') > -1) {
                    gamePropertiesObject.date =
                        '04/15/' + gamePropertiesObject.date.split(' ')[1];
                }

                if (gamePropertiesObject.date.indexOf('May') > -1) {
                    gamePropertiesObject.date =
                        '05/15/' + gamePropertiesObject.date.split(' ')[1];
                }

                if (gamePropertiesObject.date.indexOf('Jun') > -1) {
                    gamePropertiesObject.date =
                        '06/15/' + gamePropertiesObject.date.split(' ')[1];
                }

                if (gamePropertiesObject.date.indexOf('Jul') > -1) {
                    gamePropertiesObject.date =
                        '07/15/' + gamePropertiesObject.date.split(' ')[1];
                }

                if (gamePropertiesObject.date.indexOf('Aug') > -1) {
                    gamePropertiesObject.date =
                        '08/15/' + gamePropertiesObject.date.split(' ')[1];
                }

                if (gamePropertiesObject.date.indexOf('Sep') > -1) {
                    gamePropertiesObject.date =
                        '09/15/' + gamePropertiesObject.date.split(' ')[1];
                }

                if (gamePropertiesObject.date.indexOf('Oct') > -1) {
                    gamePropertiesObject.date =
                        '10/15/' + gamePropertiesObject.date.split(' ')[1];
                }

                if (gamePropertiesObject.date.indexOf('Nov') > -1) {
                    gamePropertiesObject.date =
                        '11/15/' + gamePropertiesObject.date.split(' ')[1];
                }

                if (gamePropertiesObject.date.indexOf('Dec') > -1) {
                    gamePropertiesObject.date =
                        '12/15/' + gamePropertiesObject.date.split(' ')[1];
                }
            }
            ...
            if (vm.inputIsMonthUnknown && vm.inputIsDayUnknown) {
                ...
                gamePropertiesObject.date =
                    '06/06/' + gamePropertiesObject.date;
            }
            ...
            if (vm.inputHasCompleteData) {
                ...
                vm.deathScores[0].board = boardMapper.mapLevelToBoardNumber(
                    vm.deathScores[0].board
                );
                vm.deathScores[1].board = boardMapper.mapLevelToBoardNumber(
                    vm.deathScores[1].board
                );
                vm.deathScores[2].board = boardMapper.mapLevelToBoardNumber(
                    vm.deathScores[2].board
                );
                vm.deathScores[3].board = boardMapper.mapLevelToBoardNumber(
                    vm.deathScores[3].board
                );
        ...
        function convertSumsToIndividuals() {
            ...
            sanitizedDeaths[0].board = boardMapper.mapLevelToBoardNumber(
                sanitizedDeaths[0].board
            );
            sanitizedDeaths[1].board = boardMapper.mapLevelToBoardNumber(
                sanitizedDeaths[1].board
            );
            sanitizedDeaths[2].board = boardMapper.mapLevelToBoardNumber(
                sanitizedDeaths[2].board
            );
            sanitizedDeaths[3].board = boardMapper.mapLevelToBoardNumber(
                sanitizedDeaths[3].board
            );
            ...
            for (var i = 0; i < vm.gameSumScoresMap.length; i += 1) {
                ...
                vm.gameScoresMap[i] =
                    currentBoardSumScore -
                    currentDeathPoints -
                    previousBoardSumScore;
