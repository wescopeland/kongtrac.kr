(function() {
    ...
    function EventController($stateParams, eventService, gameService) {
                ...
                .then(function then(response) {
                    ...
                    vm.eventData.games.forEach(function(game) {
                        gameService
                            ...
                            .then(function then(gameResponse) {
                                if (
                                    vm.eventData.winnings[
                                        camelize(gameResponse.player)
                                    ]
                                ) {
                                    gameResponse.winnings =
                                        vm.eventData.winnings[
                                            camelize(gameResponse.player)
                                        ];
                                }
                                ...
                                if (
                                    gameResponse.score >= 100000 &&
                                    gameResponse.score < 200000
                                ) {
                                    histogramData[0] += 1;
                                } else if (
                                    gameResponse.score >= 200000 &&
                                    gameResponse.score < 300000
                                ) {
                                    histogramData[1] += 1;
                                } else if (
                                    gameResponse.score >= 300000 &&
                                    gameResponse.score < 400000
                                ) {
                                    histogramData[2] += 1;
                                } else if (
                                    gameResponse.score >= 400000 &&
                                    gameResponse.score < 500000
                                ) {
                                    histogramData[3] += 1;
                                } else if (
                                    gameResponse.score >= 500000 &&
                                    gameResponse.score < 600000
                                ) {
                                    histogramData[4] += 1;
                                } else if (
                                    gameResponse.score >= 600000 &&
                                    gameResponse.score < 700000
                                ) {
                                    histogramData[5] += 1;
                                } else if (
                                    gameResponse.score >= 700000 &&
                                    gameResponse.score < 800000
                                ) {
                                    histogramData[6] += 1;
                                } else if (
                                    gameResponse.score >= 800000 &&
                                    gameResponse.score < 900000
                                ) {
                                    histogramData[7] += 1;
                                } else if (
                                    gameResponse.score >= 900000 &&
                                    gameResponse.score < 1000000
                                ) {
                                    histogramData[8] += 1;
                                } else if (
                                    gameResponse.score >= 1000000 &&
                                    gameResponse.score < 1100000
                                ) {
                                    histogramData[9] += 1;
                    ...
                    for (var key in vm.eventData.winnings) {
                        if (vm.eventData.winnings.hasOwnProperty(key)) {
                            vm.eventData.prizePool +=
                                vm.eventData.winnings[key];
                        }
                    }
                    ...
                    vm.histogramChartConfiguration.series = [
                        {
                            data: histogramData,
                            name:
                                vm.eventData.name.match(/[A-Z0-9#]/g).join('') +
                                ' (Histogram)',
                            color: '#000000',
                            lineWidth: 3,
                            borderWidth: 0,
        ...
        function handleEditCommit() {
            eventService
                .editEvent(vm.inputEvent, vm.eventEditData)
                .then(function() {
                    vm.isEditing = false;
                });
        }
