(function() {
    ...
    ComparePlayersController.$inject = ["$q", "$interval", "$filter", "$state", "$stateParams", "compareService", "playerService", "eventService", "gameService"];
    angular
        .module('kongtrac.compare')
        .controller('ComparePlayersController', ComparePlayersController);

    /* @ngInject */
    function ComparePlayersController(
        $q,
        $interval,
        $filter,
        $state,
        ...
        function activate() {
            ...
            $q.all(promises).then(function then(playersResponse) {
                ...
                vm.playersData.forEach(function(player) {
                    ...
                    playerService
                        .getPlayerGames(player.gameIds)
                        .then(function then(gamesResponse) {
                            player.gamesData = gamesResponse;

                            player.arcadeBest = playerService.getArcadeBest(
                                player.gamesData
                            );
                            player.mameBest = playerService.getMAMEBest(
                                player.gamesData
                            );
                            player.firstKSDate = playerService.getFirstKSDate(
                                player.gamesData
                            );
                            player.firstMillionDate = playerService.getFirstMillionDate(
                                player.gamesData
                            );
                            player.pbMap = playerService.buildPBMap(
                                player.gamesData
                            );
                                ...
                                if (game.event) {
                                    ...
                                    eventService
                                        ...
                                        .then(function then(response) {
                                            newGameTableObject.eventId =
                                                game.event;
                                            newGameTableObject.eventName =
                                                response.name;
                                            newGameTableObject.eventWinnings = response.winnings
                                                ? response.winnings[
                                                      camelize(player.name)
                                                  ]
                                                : null;
                                            newGameTableObject.eventStartDate =
                                                response.startDate;
                                            newGameTableObject.eventEndDate =
                                                response.endDate;

                                            // Find this player's position in the specified event.
                                            var eventGames = [];
                                            var pushedEventData = false;
                                            response.games.forEach(function(
                                                gameId
                                            ) {
                                                gameService
                                                    .getGameData(gameId)
                                                        ...
                                                        for (
                                                            ...
                                                            if (
                                                                ...
                                                                newGameTableObject.eventPosition =
                                                                    i + 1;
                                                                newGameTableObject.eventPositionOf =
                                                                    eventGames.length;
                                                                break;
                                                            }
                                                        }
