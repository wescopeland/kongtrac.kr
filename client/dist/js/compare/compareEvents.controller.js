(function() {
    ...
    /* @ngInject */
    function CompareEventsController($q, $filter, $state, $stateParams, compareService, eventService, gameService) {
                    ...
                    $q.all(gamesPromises).then(function then(gamesResponse) {
                        ...
                        gamesResponse.forEach(function(game) {
                            ...
                            if (event.winnings[camelize(game.player)]) {
                                game.winnings = event.winnings[camelize(game.player)];
                            }
                            ...
                            if (game.score >= 100000 && game.score < 200000) {
                                histogramData[0] += 1;
                            } else if (game.score >= 200000 && game.score < 300000) {
                                histogramData[1] += 1;
                            } else if (game.score >= 300000 && game.score < 400000) {
                                histogramData[2] += 1;
                            } else if (game.score >= 400000 && game.score < 500000) {
                                histogramData[3] += 1;
                            } else if (game.score >= 500000 && game.score < 600000) {
                                histogramData[4] += 1;
                            } else if (game.score >= 600000 && game.score < 700000) {
                                histogramData[5] += 1;
                            } else if (game.score >= 700000 && game.score < 800000) {
                                histogramData[6] += 1;
                            } else if (game.score >= 800000 && game.score < 900000) {
                                histogramData[7] += 1;
                            } else if (game.score >= 900000 && game.score < 1000000) {
                                histogramData[8] += 1;
                            } else if (game.score >= 1000000 && game.score < 1100000) {
                                histogramData[9] += 1;
                        ...
                        vm.histogramChartConfiguration.series.push({
                            data: histogramData,
                            name: (event.name).match(/[A-Z0-9#]/g).join('') + ' (Histogram)',
                            lineWidth: 3,
                            bodrerWidth: 0,
                            marker: {
                                enabled: true
