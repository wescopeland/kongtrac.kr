(function() {
    ...
    angular.module('kongtrac.event').service('eventService', eventService);
    ...
    function eventService($q, $firebaseObject, $firebaseArray) {
            ...
            return $q(function(resolve, reject) {
                removeGameIdFromEvents(inputGameId).then(function() {
                    var eventData = $firebaseObject(
                        _fbRef.child('events').child(inputEventId)
                    );
        ...
        function getEventData(inputEvent) {
            var eventData = $firebaseObject(
                _fbRef.child('events').child(inputEvent)
            );
        ...
        function removeGameIdFromEvents(inputGameId) {
            return $q(function(resolve, reject) {
                var allEvents = $firebaseArray(_fbRef.child('events'));

                allEvents.$loaded().then(function() {
                    allEvents.forEach(function(event) {
                        if (event.games) {
                            var gameRemovalIndex = event.games.indexOf(
                                inputGameId
                            );
