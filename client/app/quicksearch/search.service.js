(function() {
    ...
    angular.module('kongtrac.search').service('searchService', searchService);

    /* @ngInject */
    function searchService($q, $filter, algolia) {
        // Private Variables
        var _algoliaClient = algolia.Client(
            'NW1XKBQ46B',
            '6de01060732bc03eb72a54c7e19c600d'
        );
        ...
        function indexSearch(inputIndex, inputQuery) {
            ...
            inputIndex.search(inputQuery, { hitsPerPage: 40 }).then(
                function then(response) {
                    if (inputIndex === _gamesIndex) {
                        var sortedHits = $filter('orderBy')(
                            response.hits,
                            '-score'
                        );
                        sortedHits = $filter('filter')(sortedHits, {
